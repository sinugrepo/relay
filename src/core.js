...
//   streaming s.d. ~300 dtk). Karena itu request streaming langsung
//   mengembalikan SSE preamble TANPA menunggu fetch upstream selesai — TTFB
//   upstream (antrean/model reasoning, konteks raksasa) sering > 25 dtk dan
//   `await fetch()` sebelum Response adalah penyebab 504 beruntun di log
//   backend.
//   Niat streaming dideteksi dari DUA sinyal (salah satu cukup):
//     1. Body JSON mengandung `"stream": true` (klien Responses/chat langsung), ATAU
//     2. Header `Accept: text/event-stream` (backend SELALU memasangnya di
//        semua jalur streaming, termasuk chat-bridge yang payload Responses-nya
//        TIDAK punya field `stream` — kasus inilah yang dulu lolos ke jalur
//        non-streaming lalu mati 504 tepat 25 dtk pada konteks raksasa).
//   Non-streaming (JSON biasa, Accept: application/json) tetap menunggu fetch
//   agar status HTTP upstream (429/500/dll) diteruskan utuh untuk failover
//   backend.
...
// Batas aman buffer untuk respons non-SSE yang dibungkus jadi satu event
// (jalur bungkus di bawah). Mencegah Edge kehabisan memori bila upstream
// mengembalikan body raksasa sekaligus. 5MB di bawah batas payload 4.5MB
// masih longgar untuk JSON Responses normal (< 1MB).
...
/**
 * Deteksi niat streaming. Dua sinyal, salah satu cukup:
 *   - Body JSON dengan `stream === true` (batas baca 2MB; di atas itu body
 *     dianggap non-stream agar tidak boros memori — sinyal Accept di bawah
 *     tetap menutupi kasus konteks raksasa), ATAU
 *   - Header `Accept: text/event-stream` (dikirim backend di SEMUA jalur
 *     streaming, termasuk chat-bridge yang payload-nya tanpa field `stream`).
 * @param {Request} request
 * @param {ArrayBuffer|undefined} bodyBuf
 * @returns {boolean}
 */
function wantsStreamResponse(request, bodyBuf) {
  try {
    const accept = (request.headers.get("accept") || "").toLowerCase();
    if (accept.includes("text/event-stream")) return true;
  } catch {
    // Header tak terbaca: jatuh ke deteksi body di bawah.
  }
  return isStreamRequest(bodyBuf);
}
...
export async function handleRelay(request) {
  ...
  // upstream > 25 dtk (muse-spark reasoning/antrean/konteks raksasa), Edge
  // dibunuh sebelum preamble sempat dikirim. Di sini byte pertama (< 1 ms) +
  // heartbeat 10 dtk menjaga Edge tetap hidup s.d. ~300 dtk sambil menunggu
  // upstream. Deteksi via body ATAU header Accept (lihat wantsStreamResponse).
  if (wantsStreamResponse(request, body)) {
    ...
    (async () => {
      ...
      if (!contentType.includes("text/event-stream") || !upstream.body) {
        ...
        // Buffer DIBATASI (baca via reader + abort) agar body raksasa tidak
        // meledakkan memori Edge; kelebihannya dilaporkan sebagai relay.error
        // supaya backend failover ke target berikutnya.
        try {
          ...
          if (tooLarge) {
            await safeWrite(
              encoder.encode(relayErrorEvent(502, "Relay buffered body too large"))
            );
          } else {
